...
"""def doc_handler(path):
    ...
    list_details = folderName(path)
    try:
        file_name = ".".join(path.split("/")[-1].split(".")[0:-1]) + ".pdf"
        out_file = os.path.join(temp_storage_path, file_name)
        os.chdir(temp_storage_path)
...
if __name__ == '__main__':
    ...
    print('code for ppt handler has been updated ')
    ...
    try:
        ...
        if upload_to_s3(output_file, bucket_name, out):
            ...
            url = 'https://itduediligence.pwcinternal.com/dataRoom/ProcessEmailRequest/'
